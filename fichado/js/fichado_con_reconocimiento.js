...
document.addEventListener('touchstart', function (event) {
  if(document.querySelector('main').style.display == 'block') limpiarCampos()
  if (arranca_fichar == 1) return
  if(event.target.id == 'enter') {
      let doc = document.getElementById('documento').value
      iniciarFichado(doc.trim(),1)
  }
})
...
function iniciarFichado(doc,op) {
  ...
  .then(function (json) {
    ...
    if (json.error == 1) {
      ...
      datos_persona = json.datos[0]
    }
    
    if (documento == doc) intentos++
    else intentos = 0
    documento = doc
    ...
    if (op == 1) {
      ...
      if (window.innerHeight > 1200) { 
        const audio = new Audio('wav/comparacion_facial_recuadro.mp3')
        audio.play()
        recuadro()
      }else{
